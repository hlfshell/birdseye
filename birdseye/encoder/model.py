...
def Encoder():
    ...
    outputs = ResNet50(include_top=False, weights="imagenet", input_shape=input_shape)(
        inputs
    )
...
def Decoder2():
    ...
    # 3 filters for 3 channels of our output
    outputs = Conv2D(3, 3, padding="same", activation="tanh")(net)
...
def Critic2():
    ...
    for filters in [1024, 512, 256, 128, 64, 32, 16]:
        net = Conv2D(filters, 3, padding="same", activation="leaky_relu")(net)
        net = Conv2D(filters, 3, padding="same", activation="leaky_relu")(net)

        # Skip layer
        skip_memory = Conv2D(filters, 1, pading="same", activation="leaky_relu")(
            skip_memory
        )
        net = add([net, skip_memory])
        skip_memory = net
